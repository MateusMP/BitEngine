...
bool CommandSystem::registerKeyCommandForAllMods(int commandID, int commandState, int key)
{
    ...
    u32 s = 1;
    u32 f = 0;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::PRESS, KeyMod::NONE)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::PRESS, KeyMod::ALT)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::PRESS, KeyMod::CTRL)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::PRESS, KeyMod::SHIFT)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::PRESS, KeyMod::SUPER)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::PRESS, KeyMod::ALT_SHIFT)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::PRESS, KeyMod::CTRL_ALT)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::PRESS, KeyMod::CTRL_SHIFT)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::PRESS, KeyMod::SUPER_ALT)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::PRESS, KeyMod::SUPER_CTRL)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::PRESS, KeyMod::SUPER_SHIFT)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::PRESS, KeyMod::ALT_SHIFT_SUPER)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::PRESS, KeyMod::SHIFT_CTRL_SUPER)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::PRESS, KeyMod::CTRL_ALT_SHIFT)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::PRESS, KeyMod::CTRL_ALT_SUPER)) { f |= s; } s <<= 1;

    if (f != 0x7FFF) {
        LOG(EngineLog, BE_LOG_WARNING) << "Failed to register key for all press mods: " << f << ". Ambiguos key?";
    }

    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::RELEASE, KeyMod::NONE)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::RELEASE, KeyMod::ALT)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::RELEASE, KeyMod::CTRL)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::RELEASE, KeyMod::SHIFT)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::RELEASE, KeyMod::SUPER)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::RELEASE, KeyMod::ALT_SHIFT)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::RELEASE, KeyMod::CTRL_ALT)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::RELEASE, KeyMod::CTRL_SHIFT)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::RELEASE, KeyMod::SUPER_ALT)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::RELEASE, KeyMod::SUPER_CTRL)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::RELEASE, KeyMod::SUPER_SHIFT)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::RELEASE, KeyMod::ALT_SHIFT_SUPER)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::RELEASE, KeyMod::SHIFT_CTRL_SUPER)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::RELEASE, KeyMod::CTRL_ALT_SHIFT)) { f |= s; } s <<= 1;
    if (registerKeyboardCommand(cmd, cmdS, key, KeyAction::RELEASE, KeyMod::CTRL_ALT_SUPER)) { f |= s; } s <<= 1;
