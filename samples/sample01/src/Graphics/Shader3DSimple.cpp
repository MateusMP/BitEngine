...
Shader3DSimple::Model::Model(const std::string& baseDirectory){
	...
	LOG(GameLog(), BE_LOG_VERBOSE) << "MODEL CREATED: " << this;
}
...
typename Shader3DSimple::Mesh* Shader3DSimple::Model::processMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	{
		return new Mesh(mesh->mName.C_Str(), vertices, indices, static_cast<Shader3DSimple::Material*>(materials[mesh->mMaterialIndex]) );
	}
	else
	{
		LOG(GameLog(), BE_LOG_ERROR) << "Model has no material!";

		return nullptr;
	}
}

Shader3DSimple::Material* Shader3DSimple::Model::createMaterial(BitEngine::ResourceLoader* tMng, const aiMaterial* material){
...
//		mat->diffuse = tMng->getResource<BitEngine::Texture>(m_baseDir + std::string(path.C_Str()));
	...
	{
		LOG(GameLog(), BE_LOG_WARNING) << "unexpected number of diffuse textures: " << material->GetTextureCount(aiTextureType_NORMALS);
	}
...
//		mat->normal = tMng->getResource<BitEngine::Texture>(m_baseDir + std::string(path.C_Str()));
	...
	{
		LOG(GameLog(), BE_LOG_WARNING) << "unexpected number of normal textures: " << material->GetTextureCount(aiTextureType_NORMALS);
	}
